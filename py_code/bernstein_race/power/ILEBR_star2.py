...
if __name__ == "__main__":
    with WorkerPool(n_jobs=8) as pool:
        ...
        x1_ILEBR_star2 = np.array(
            [pool.map(ILEBR_star2_test, [p] * 2000) for p in ps1])
        np.save("powers/ILEBR_star2_powr", x1_ILEBR_star2)

        ps2 = np.arange(0, 0.5 + 0.05, 0.05)
        x2_ILEBR_star2 = np.array(
            [pool.map(ILEBR_star2_test, [p] * 200) for p in ps2])
        ...
        # x1_ILEBR_star2 = []
        # for p in ps1:
        #     x1_ILEBR_star2.append(pool.map(ILEBR_star2_test, [p]*1))
        # x1_ILEBR_star2 = np.array(x1_ILEBR_star2)
        # np.save("powers/ILEBR_star2_powr", x1_ILEBR_star2)
        # x2_ILEBR_star2 = []
        # for p in ps2:
        #     x2_ILEBR_star2.append(pool.map(ILEBR_star2_test, [p]*1))
