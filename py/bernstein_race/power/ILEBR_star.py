...
with WorkerPool(n_jobs=4) as pool:
    ...
    # x2_ILEBR_star = np.array([pool.map(ILEBR_star_test, [p]*200) for p in ps2])
    ...
    x1_ILEBR_star = []
    for p in ps1:
        x1_ILEBR_star.append(pool.map(ILEBR_star_test, [p]*1, progress_bar=True))
    x1_ILEBR_star = np.array(x1_ILEBR_star)
    np.save("powers/ILEBR_star_powr", x1_ILEBR_star)
    x2_ILEBR_star = []
    for p in ps2:
        x2_ILEBR_star.append(pool.map(ILEBR_star_test, [p]*10))
